...
#include <stdio.h>
#include "nanocomp.h"
//  Convert PNG image with palette to SREC file with Palette at 4000 and paged image at 8000-BFFF, upto 4 pages 16K each, 64K total
//  gcc pngtosrec.c spng.c miniz.c -o pngtosrec -lm
// ./pngtosrec Doom.png > doom.srec
...
int main(int argc, char **argv)
{
    ...
    int pallete_add = 0x4000;
    for(j=0; j < plte.n_entries; j++)
    {
        checksum = ~((pallete_add / 256) + (pallete_add & 0xFF) + 6 + (plte.entries[j].red >> 2) + (plte.entries[j].green >> 2) + (plte.entries[j].blue >> 2) );
        printf("S1%02X%04X%02X%02X%02X%02X\n\r", 6, pallete_add, (plte.entries[j].red  >> 2), (plte.entries[j].green >> 2), (plte.entries[j].blue >> 2), checksum);
        pallete_add += 3;
    }
    ...
    printf("spng_decode_image_size() Size %ld\n", image_size);
    ...
    int vram_address = 0x8000;
    
    int page_address = 0xD480;
    
    int page = 0;
    unsigned char *imageByte = image;
    unsigned char *imageByte2 = image + 320;
    
    checksum = ~((page_address / 256) + (page_address & 0xFF) + 4 + (unsigned char) (page + VIDEO_MODE3));
    printf("S1%02X%04X%02X%02X\n", 4, page_address, (unsigned char) (page + VIDEO_MODE3), checksum);

    // Read bytes two rows of 320 at a time until end of file
    for ( unsigned int i = 0; i < image_size; i += 640){
      // Do 40 groups of 16 bytes in 4 groups of 4 bytes on 2 lines 
      for ( int j=0; j < 40; j++){
          checksum = (vram_address / 256) + (vram_address & 0xFF) + 19;
          printf("S1%02X%04X", 19, vram_address);
          for (int k = 0; k < 4; k++){
            checksum += *imageByte;
            printf("%02X", *imageByte++);
          }
          for (int k = 0; k < 4; k++){
            checksum += *imageByte2;
            printf("%02X", *imageByte2++);
          }
          for (int k = 0; k < 4; k++){
            checksum += *imageByte;
            printf("%02X", *imageByte++);
          ...
          printf("%02X\n\r", (unsigned char) ~checksum);
          vram_address += 16;
          if (vram_address >= 0xC000){
            vram_address = 0x8000;
            page++;
            checksum = ~((page_address / 256) + (page_address & 0xFF) + 4 + (unsigned char) (page + VIDEO_MODE3));
            printf("S1%02X%04X%02X%02X\n", 4, page_address, (unsigned char) (page + VIDEO_MODE3), checksum);
    ...
    // Set page back to 0
    checksum = ~((page_address / 256) + (page_address & 0xFF) + 4 + (unsigned char) (VIDEO_MODE3));
    printf("S1%02X%04X%02X%02X\n", 4, page_address, (unsigned char) (VIDEO_MODE3), checksum);

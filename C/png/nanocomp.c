...
{                                                                       // Does not wait for !H_SYNC
   ...
   *pageRegister = (byte)(GMODE2 + page);
   ...
   while (true){
     *vramAddress = clear;
     vramAddress++;
     if ((word) vramAddress >= (VRAMPAGE3TOP)) break;
   }
   page = 0;
   *pageRegister = (byte)(GMODE2 + page);
...
void graphicsInit()
{
  byte CRTC_REGISTERS[16]={
                      0x63,         // R0 H 62 to 64 Total 98 was 99 (Changed to 98 with 25Mhz as 59Hz not 60Hz)
                      ...
                      0x00,0x00};   // R14,R15 Cursor Address
  ...
  for (int r = 0; r <= 15; r++)
  {
    ...
    *CRTCDataRegister = (byte) CRTC_REGISTERS[r];
  }

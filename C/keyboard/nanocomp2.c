#include "nanocomp2.h"
// Nanocomp2.c has scan code conversion for keyboard and uses 82C42 conversion of scan set 2 to set 1
...
int keyboardInit(){
 ...
 //		               |`-- 1 == enable scan code translation to IBM PC/XT scan codes 0=AT
 //		               `-- 0 == reserved, must be 0
...
// Waits until key pressed scan code is received then looks up, ignores BREAK codes > 0x80
word get_kbd_char(){
  word returnWord = 0x0000;

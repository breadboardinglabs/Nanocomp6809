...
DISPENDADDRESS                RMB 2; Current VRAM End Address value = VRAM + 30 x 80 x 2 = 4800 $12C0
DISPCURSORH                   RMB 1;
DISPCURSORL                   RMB 1;
...
DISABLECURSOR                 EQU  $20   ; Bits needed to disable cursor
...
CRTCRAMLOOP                   lda      ,X+               ; Get Rn value from CRTC Table single + for +1, ++ is +2 and wrong!
                              ...
                              ldd      #VRAM+COLSPERROW*ROWSPERPAGE*BYTESPERCHAR-1 ; VRAM + 4799 or $12BF for 30x80
                              std      DISPENDADDRESS    ; Need this for more efficient clear screen
...
CRTCLOOP                      stb      CRTCADDRESS       ; Save Rn in CRTC Address Register        
                              ...
                              cmpb     #$10              ; Have we processed all register values
                              bne      CRTCLOOP
                              lbsr      COPYCHARGENROM    ; Copy Character bitmap values from ROM to CG RAM
                              lbsr      CLRSCREEN
                              ldy      #BOOTMESSAGE
                              ldx      #VRAM+1           ; First Character of Video RAM
                              lbsr      DISPMSGY
                              rts
...
CHROUT                        pshs     a,b,x
                              ...
                              cmpb     #asciiCR
                              bgt      CHROUTRAWINT  ; Continue to display characters higher than CR
                              bne      NOTCR
                              clr      DISPCOL ; Carriage Return so set DISPCOL to 0 for next character
                              bra      CHROUTEND ; Skip display of character

NOTCR                         cmpb     #asciiLF ; LF we wil increase line and set DISPCOL to 0
                              bne      NOTLF
                              pshs     a      ; Make another copy of A                      
                              clr      DISPCOL ; Line Feed so set DISPCOL to 0 for next character and increase row unless end of page
                              lda      #ROWSPERPAGE-1
                              cmpa     DISPROW ;  Check is there enough space below
                              bhi      SPACEONPAGE2 ; Space so inc ROW
                              clr      DISPROW ; Reached end of page, go back to start
                              bra      PULACONTINUE ; DISPROW will already be 0
SPACEONPAGE2                  inc      DISPROW ; LF with space on page
PULACONTINUE                  puls     a         ; Restore A
                              bra      CHROUTEND ; Skip display of character
...
NOTFF                         cmpb     #asciiBS
                              bne      NOTBS
                              tst      DISPCOL   ; Test if COL is 0 then don't decrement
                              beq      NOTBS
                              dec      DISPCOL
                              bra      CHROUTEND
                              
NOTBS                         bra      CHROUTRAWINT ; Process other characters from 0-13 which are not BS, LF, FF, CR
...
CHROUTRAW                     pshs     a,b,x
CHROUTRAWINT                  lbsr     ROWCOLTOX ; Gets X pointing to colour for DISPROW, DISPCOL, internal entry from CHROUT
                              tsta
                              beq      NOCOLOUR  ; If A is 0 then don't update colour
                              std      ,x
                              bra      CHECKENDOFROW
NOCOLOUR                      stb      1,x
CHECKENDOFROW                 lda      #COLSPERROW-1  ; Control Characters from CHROUT will join here
                              cmpa     DISPCOL
                              bhi      SPACEONLINE ; We have enough space on line so inc DISPCOL
                              clr      DISPCOL ; End of line so set DISPCOL to 0 for next character
                              lda      #ROWSPERPAGE-1
                              cmpa     DISPROW ; New row but is there enough space below
                              bhi      SPACEONPAGE
                              clr      DISPROW ; Reached end of page, go back to start
                              bra      CHROUTEND ; DISPROW will already be 0 so no need to inc                       
SPACEONPAGE                   inc      DISPROW
                              bra      CHROUTEND ; DISPCOL will already be 0 so no need to inc                       
SPACEONLINE                   inc      DISPCOL
CHROUTEND                     puls    a,b,x,pc                              
...
LINEMSGY                      ldb      ,y+
                              beq      LINEMSGYEND  ; End if Null value terminating string
                              lbsr     ROWCOLTOX ; X now points to colour byte
                              lda      ,x
                              bsr      CHROUTRAW ; Output the colour from A and Character from B
                              ble      LINEMSGY
LINEMSGYEND                   rts 
                              ...
                              ; Does not handle CR/LF or FF/Clear Screen or update DISP Variables, for OS info etc
DISPMSGY                      lda      ,y+
                              beq      DISPMSGYEND  ; End if Null value terminating string
                              sta      ,x++
                              nop                 ; Allow SWI to be entered and continue
                              cmpx     DISPENDADDRESS ; Don't allow overwriting end of screen buffer
                              ble      DISPMSGY
DISPMSGYEND                   rts 

                              ; Clearscreen and initalise screen DISP RAM variables
CLRSCREEN                     pshs     a,b,x,y
                              ldx      #VRAM
                              ldd      DISPCLS ; Get default colour and character, A MSB, B LSB
CLEARSCREEN1                  lbsr     WAITFORVSDE ; Wait for vertical blanking period
                              ldy      #80 ; Do 1 line per blanking period of 1.6ms
CLEARSCREEN2                  std      ,X++
                              leay     -1,Y
                              bne      CLEARSCREEN2
                              cmpx     DISPENDADDRESS
                              ...
                              clr      DISPCURSORL
                              ldd      #VRAM+COLSPERROW*ROWSPERPAGE*BYTESPERCHAR-1 ; VRAM + 4799 or $12BF for 30x80
                              addd     DISPSTARTADDRESS ; Add the CRTC Start offset to give real end address
                              std      DISPENDADDRESS
...
DISABLECRTCCURSOR             pshs     a
                              lda      #$0A                ; R10
                              sta      CRTCADDRESS
                              lda      #DISABLECURSOR      ;DISABLECURSOR = $20
                              sta      CRTCDATAREGISTER
                              puls     a,pc
...
ROWCOLTOX                     pshs     a,b
                              ldx      #COLADDRESS ; Get base added for Column Address table
                              lda      DISPROW     ; Get row which is used for offset
                              ...
                              ldb      DISPCOL     ; Now get the column (X)
                              ...
                              ldd      DISPSTARTADDRESS ; Get CRTC start address if used for scrolling in frame buffer
                              leax     d,x         ; If scrolling in VRAM then add CRTC offset, d will be low enough not to worry about sign
                              puls     a,b,pc
...
COLADDRESS                    FDB      #VRAM
COLADDRESS1                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR
COLADDRESS2                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 2
COLADDRESS3                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 3
COLADDRESS4                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 4
COLADDRESS5                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 5
COLADDRESS6                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 6
COLADDRESS7                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 7
COLADDRESS8                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 8
COLADDRESS9                   FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 9
COLADDRESS10                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 10
COLADDRESS11                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 11
COLADDRESS12                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 12
COLADDRESS13                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 13
COLADDRESS14                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 14
COLADDRESS15                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 15
COLADDRESS16                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 16
COLADDRESS17                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 17
COLADDRESS18                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 18
COLADDRESS19                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 19
COLADDRESS20                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 20
COLADDRESS21                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 21
COLADDRESS22                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 22
COLADDRESS23                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 23
COLADDRESS24                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 24
COLADDRESS25                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 25
COLADDRESS26                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 26
COLADDRESS27                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 27
COLADDRESS28                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 28
COLADDRESS29                  FDB      #VRAM + COLSPERROW*BYTESPERCHAR * 29
...
BOOTMESSAGE                   FCC  "Nanocomp 6809 V10.00"
                              FCB  $00, $00, $00, $00, $00
